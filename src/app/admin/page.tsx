...
const miniMapTables = [
  ...
  { id: '68', type: 'rect-h-4', top: 4,    left: (71   - OL)*S+1, w: 5.5 *S, h: 5.5 },
...
const getOccupiedTableIdsBase = (reservations: any[], dateStr: string, timeStr: string, excludeResId?: string) => {
  ...
  const SESSION_DURATION = 120; // 2時間の滞在（管理画面は2時間のまま。お客様フォームは2時間半）
  const ids: string[] = [];
...
function MobileAdminView(props: MobileAdminViewProps) {
  const {
    ...
    setSelectedRes, setEditTime, setEditGuests, setEditTable, setEditSelectedGroup, setEditFreeTableIds, setEditNotes,
  ...
  const openReservation = (r: any) => {
    ...
    setEditNotes(getCleanNotes(r.notes));
  };
...
export default function AdminPage() {
  ...
  const getOccupiedTableIds = (dateStr: string, timeStr: string, excludeResId?: string) => {
    const targetMin = timeToMinutes(timeStr);
    const SESSION_DURATION = 120; // 2時間重複判定（管理画面は2時間のまま。お客様フォームは2時間半）
    const ids: string[] = [];
  ...
  const handleTableClick = async (table: TableStatus) => {
    ...
    if (table.isOccupied) {
      ...
      if (foundRes) {
        ...
        setEditNotes(getCleanNotes(foundRes.notes));
      }
    } else {
 ...
 const handleCancelReservation = async (id: string, guestName: string) => {
  ...
  const handleUpdateReservation = async (e: React.FormEvent) => {
    ...
    if (editGuestsNum >= 9) {
      ...
      finalNotes = buildCombinedNotes(editSelectedGroup, editNotes);
    } else {
      if (occupiedIds.includes(String(editTable).trim())) {
  ...
  return (
    <div className="p-2 min-h-screen font-sans transition-colors duration-300 bg-slate-50 text-slate-900">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 text-slate-100">
          <form onSubmit={handleUpdateReservation} className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden max-h-[90dvh] flex flex-col">
            <div className="bg-gradient-to-r from-slate-800 to-slate-900 p-4 text-white flex justify-between items-center border-b border-slate-700 shrink-0">
              ...
              <button
                type="button"
                onClick={() => { setSelectedRes(null); setEditSelectedGroup(null); setEditFreeTableIds([]); setEditNotes(''); }}
                className="w-8 h-8 rounded-full bg-slate-800 hover:bg-slate-700 text-white font-bold flex items-center justify-center transition"
                style={{ cursor: 'pointer' }}
              >
                ✕
            ...
            <div className="p-4 space-y-3.5 text-lg overflow-y-auto overscroll-contain flex-1 min-h-0">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 font-medium leading-relaxed placeholder:text-slate-600 placeholder:italic placeholder:font-normal focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
                />
              </div>
            </div>
      ...
      {showNewOrderModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 text-slate-100">
          <div className={`bg-slate-900 border border-slate-700 w-full ${newOrderStep === 'details' ? 'max-w-5xl' : 'max-w-md'} rounded-2xl shadow-2xl overflow-hidden relative`}>
            ...
            {newOrderStep === 'details' && (
              <form onSubmit={handleCreateNewOrder}>
                ...
                <div className="p-4 grid grid-cols-2 gap-5 text-lg overflow-y-auto max-h-[75vh]" style={{ WebkitOverflowScrolling: 'touch' }}>
